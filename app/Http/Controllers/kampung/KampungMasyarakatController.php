...
class KampungMasyarakatController extends Controller
{
    public function index() {
        ...
        $users = User::whereIn('id', $villageUser->select('user_id'))->orderBy('id', 'DESC');
    ...
    public function waiting() {
        $villageUser = VillageUser::where('village_id', '=', (Village::where('admin_id', '=', auth()->user()->id)->firstOrFail()->id));
        $users = User::whereIn('id', $villageUser->select('user_id'))->where('status', '=', 'Waiting')->orderBy('id', 'DESC');
    ...
    public function granted() {
        $villageUser = VillageUser::where('village_id', '=', (Village::where('admin_id', '=', auth()->user()->id)->firstOrFail()->id));
        $users = User::whereIn('id', $villageUser->select('user_id'))->where('status', '=', 'Granted')->orderBy('id', 'DESC');
